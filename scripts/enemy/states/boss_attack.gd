...
var attack_patterns: Dictionary = {
    AttackType.BASIC: {"windup": 0.4, "attack": 0.2, "recovery": 0.6, "damage": 20, "range": 2.0},
    AttackType.HEAVY: {"windup": 0.8, "attack": 0.3, "recovery": 1.0, "damage": 35, "range": 2.5},
    AttackType.AOE: {"windup": 1.0, "attack": 0.5, "recovery": 1.5, "damage": 25, "range": 4.0},
}
    ...
    match current_phase:
        AttackPhase.WINDUP:
            if attack_timer <= 0.0:
                ...
                attack_timer = attack_patterns[current_attack]["attack"]
        ...
        AttackPhase.ATTACK:
            ...
            if attack_timer <= 0.0:
                ...
                attack_timer = attack_patterns[current_attack]["recovery"]
        ...
        AttackPhase.RECOVERY:
            if attack_timer <= 0.0:
                if combo_count > 0:
                    _choose_attack()
                    _start_attack()
                elif enemy.player_ref and enemy.global_position.distance_to(enemy.player_ref.global_position) <= enemy.attack_range:
...
func _try_deal_damage() -> void:
    ...
    if distance <= pattern["range"]:
        var damage: int = pattern["damage"]
        var boss: BossEnemy = enemy as BossEnemy
        
        if boss and boss.is_enraged:
            damage = int(damage * boss.rage_mode_damage_multiplier)
        
        var player: Player = enemy.player_ref
        if player.hurtbox.monitoring:
            player.take_damage(damage)
            print("Boss dealt %d damage with %s attack" % [damage, AttackType.keys()[current_attack]])