## Heavy charge attack — windup → charge → recovery.
## Uses HeavyEnemy stats if available, otherwise fallback.
extends EnemyState
...
func enter() -> void:
    ...
    if enemy.player_ref:
        ...
        enemy.rotation.y = atan2(
            charge_direction.x, charge_direction.z
        )
...
func _process_charge() -> void:
    var heavy: HeavyEnemy = enemy as HeavyEnemy
    var spd: float = (
        heavy.charge_speed if heavy else charge_speed
    )

    enemy.velocity.x = charge_direction.x * spd
    enemy.velocity.z = charge_direction.z * spd
...
func _check_collision() -> void:
    ...
    if dist <= enemy.attack_range:
        ...
        if target.hurtbox.monitoring:
            var heavy: HeavyEnemy = enemy as HeavyEnemy
            var dmg: int = (
                heavy.charge_damage if heavy
                else charge_damage
            )
            target.take_damage(dmg)
            print("Heavy charge hit for %d damage" % dmg)